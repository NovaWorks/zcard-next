// Package audit 审计与风控模块（P2-06 完整落地）：
//   - 操作审计（T2）：变更类 admin 操作（POST/PUT/DELETE）自动落 audit_logs；
//     before/after 快照经注册表采集（各模块声明聚合键）；异步落库失败不阻断
//   - 安全审计（T3）：port.Auditor 埋点（登录/解密/导出/权限变更/黑名单变更/
//     取货）——写失败不阻断业务（1.x 纪律）；取货审计含谁/何时/IP/订单（零明文）
//   - 风控闸门（T4）：port.RiskGate——IP 黑名单（精确+CIDR，进程内缓存）→
