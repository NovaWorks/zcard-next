...
// TestGroupCatalog 分组目录快照（19 组； 验收「全部 12+ 分组」； 加 license；客服加 service；供货充值加 supplier_recharge；在线更新加 system）。
func TestGroupCatalog(t *testing.T) {
	got := GroupsSorted()
	want := []string{"affiliate", "footer", "i18n", "license", "notify", "ops", "points", "promo", "recharge", "security", "service", "site", "supplier_recharge", "supply", "system", "template", "ticket", "trade", "withdraw"}
	if len(got) != len(want) {
