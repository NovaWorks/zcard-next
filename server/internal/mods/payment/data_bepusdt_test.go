...
func beTestChannel(t *testing.T, r *PaymentRepoImpl, gateway string, modes ...string) *ent.PaymentChannel {
	t.Helper()
	mode := "fixed"
	if len(modes) > 0 {
		mode = modes[0]
	}
	cfg, _ := json.Marshal(map[string]any{"api_url": gateway, "api_token": "contract-test-token", "timeout": 600, "trade_type": "usdt.trc20", "checkout_mode": mode})
...
func TestBepusdtAttemptReuseAndConcurrentClicks(t *testing.T) {
	for _, mode := range []string{"fixed", "cashier"} {
		t.Run(mode, func(t *testing.T) {
			...
			defer gateway.Close()
			ch := beTestChannel(t, repo, gateway.URL, mode)
...
func TestBepusdtLostResponseRecoveryAndConfigGuard(t *testing.T) {
	for _, mode := range []string{"fixed", "cashier"} {
		t.Run(mode, func(t *testing.T) {
			...
			for _, patch := range []map[string]any{{"checkout_mode": "cashier", "currencies": []string{"USDC"}}, {"checkout_mode": "fixed", "trade_type": "usdt.solana"}} {
				var changedConfig map[string]any
				_ = json.Unmarshal([]byte(raw), &changedConfig)
				for k, v := range patch {
					changedConfig[k] = v
				}
				encoded, _ := json.Marshal(changedConfig)
				if _, err := repo.UpdateChannel(ctx, ch.ID, "", string(encoded), -1, "", true, -1, false, "", false, nil); err == nil || !strings.Contains(err.Error(), "CHANNEL_BUSY") {
					t.Fatalf("mode/currency change: %v", err)
				}
			}
