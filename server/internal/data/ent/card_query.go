...
func (_q *CardQuery) QueryProduct() *ProductQuery {
...
func (_q *CardQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *CardQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *CardQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *CardQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *CardQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *CardQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *CardQuery) WithProduct(opts ...func(*ProductQuery)) *CardQuery {
	query := (&ProductClient{config: _q.config}).Query()
...
func (_q *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes, nil,
			func(n *Card, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CardQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Card, init func(*Card), assign func(*Card, *Product)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Card)
	for i := range nodes {
...
func (_q *CardQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(card.Table, card.Columns, sqlgraph.NewFieldSpec(card.FieldID, field.TypeUint64))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProduct != nil {
			_spec.Node.AddColumnOnce(card.FieldProductID)
		}
	}
