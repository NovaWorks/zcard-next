...
type Tag struct {
	...
	Position tag.Position `json:"position,omitempty"`
...
func (*Tag) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case tag.FieldName, tag.FieldSlug, tag.FieldIcon, tag.FieldColor, tag.FieldPosition:
...
func (_m *Tag) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case tag.FieldPosition:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field position", values[i])
			} else if value.Valid {
				_m.Position = tag.Position(value.String)
			}
		case tag.FieldHide:
