...
func (_q *ProductQuery) QueryCards() *CardQuery {
...
func (_q *ProductQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *ProductQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *ProductQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *ProductQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *ProductQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *ProductQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ProductQuery) WithCards(opts ...func(*CardQuery)) *ProductQuery {
	query := (&CardClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) loadCards(ctx context.Context, query *CardQuery, nodes []*Product, init func(*Product), assign func(*Product, *Card)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(card.FieldProductID)
	}
	query.Where(predicate.Card(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.CardsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
