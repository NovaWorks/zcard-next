...
func wireApp(serverConf *conf.Server, dataConf *conf.Data, securityConf *conf.Security, logger *slog.Logger) (*appDeps, func(), error) {
	...
	storeTicketService := ticket.NewStoreTicketService(ticketRepo, generator, portWallet, repoImpl, outboxWriter, logger)
	...
	adminCatalogService := catalog.NewAdminCatalogService(productRepoImpl, orderUsecase, portSettingsReader, cardCipher)
	...
	storeCartService := order.NewStoreCartService(dataData, productRepoImpl, cardRepoImpl, repoImpl, couponRepoImpl)
