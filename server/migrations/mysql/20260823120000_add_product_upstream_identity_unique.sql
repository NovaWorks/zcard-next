...
DELETE p FROM `products` p JOIN `products` q
  ON p.`subsite_id` = q.`subsite_id` AND p.`upstream_source_id` = q.`upstream_source_id` AND p.`upstream_product_code` = q.`upstream_product_code`
WHERE p.`id` > q.`id` AND p.`upstream_product_code` IS NOT NULL;
