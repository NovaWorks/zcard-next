    ...
    <NModal v-model:show="showCreate" preset="dialog" title="新增支付渠道" style="width: 560px">
      <NForm :model="form" label-placement="left" label-width="90">
        ...
        <NFormItem label="凭据 JSON" required>
          <NInput v-model:value="form.config_json" type="textarea" :rows="5" placeholder='{"app_id":"...","private_key":"..."}' />
        </NFormItem>
        <NFormItem label="启用">
...
async function handleCreate() {
  ...
  try {
    ...
    if (!error) {
      ...
      Object.assign(form, { name: '', code: '', driver: 'wallet', config_json: '{}', enabled: true });
      loadList();
    }
