  ...
  <div class="min-h-500px flex-col gap-16px overflow-hidden">
    <NCard title="商品管理" class="flex-1">
      <div class="mb-16px flex items-center gap-12px">
        ...
        <NInput v-model:value="keyword" placeholder="搜索商品名" clearable class="w-200px" @keyup.enter="loadList" />
    ...
    <!-- 新增/编辑弹窗 -->
    <NModal v-model:show="showCreate" preset="dialog" :title="editingId ? '编辑商品' : '新增商品'" style="width: 600px">
...
const columns: DataTableColumns<any> = [
  ...
  },
  {
    ...
    width: 100,
    render: (row) => row.factory_price_cents ? `¥${(row.factory_price_cents / 100).toFixed(2)}` : '-'
  },
  {
    title: '类型',
    key: 'stock_type',
  ...
  },
  {
    ...
    key: 'status',
    width: 80,
    render: (row) => h(NTag, { type: row.status === 1 ? 'success' : row.status === 2 ? 'warning' : 'default', size: 'small' }, {
